export type FieldServiceCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildFieldServiceCoreSqliteMigrationSql(options: FieldServiceCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "field_service_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getFieldServiceCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getFieldServiceCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildFieldServiceCoreSqliteRollbackSql(options: FieldServiceCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "field_service_core_");
  return [
...
export function getFieldServiceCoreSqliteLookupIndexName(tablePrefix = "field_service_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getFieldServiceCoreSqliteStatusIndexName(tablePrefix = "field_service_core_"): string {
