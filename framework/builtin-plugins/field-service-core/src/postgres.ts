export type FieldServiceCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildFieldServiceCoreMigrationSql(options: FieldServiceCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "field_service_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getFieldServiceCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getFieldServiceCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildFieldServiceCoreRollbackSql(options: FieldServiceCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "field_service_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "field_service_core";
  return [
...
export function getFieldServiceCoreLookupIndexName(): string {
  return "field_service_core_primary_lookup_idx";
}

export function getFieldServiceCoreStatusIndexName(): string {
  return "field_service_core_exception_status_idx";
}
