...
import { businessFlowDefinitions, scheduleDispatch, startFieldVisit, requestSpareParts, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("field-service-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("field-service.dispatches.schedule");
    expect(manifest.publicEvents).toContain("field-service.dispatch-scheduled.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof scheduleDispatch).toBe("function");
    expect(typeof startFieldVisit).toBe("function");
    expect(typeof requestSpareParts).toBe("function");
